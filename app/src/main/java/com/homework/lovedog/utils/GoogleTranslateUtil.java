...
//                e.put(f++, h >> 6 | 192);
//            } else if (55296 == (h & 64512) && g + 1 < a.length() && 56320 == (a.charAt(g + 1)
//            & 64512)) {//(55296 == (h & 64512) && g + 1 < a.length && 56320 == (a.charCodeAt(g
//            + 1) & 64512) ? (h = 65536 + ((h & 1023) << 10) + (a.charCodeAt(++g) & 1023),
//            e[f++] = h >> 18 | 240, e[f++] = h >> 12 & 63 | 128) :
...
//    }
    ...
    public static String sendGet(String url) {
        ...
        try {
            ...
            connection.setRequestProperty("cookie", "NID=188" +
                "=M1p_rBfweeI_Z02d1MOSQ5abYsPfZogDrFjKwIUbmAr584bc9GBZkfDwKQ80cQCQC34zwD4ZYHFMUf4F59aDQLSc79_LcmsAihnW0Rsb1MjlzLNElWihv-8KByeDBblR2V1kjTSC8KnVMe32PNSJBQbvBKvgl4CTfzvaIEgkqss");
            connection.setRequestProperty("referer", "https://translate.google.cn/");
            connection.setRequestProperty("x-client-data",
                "CJK2yQEIpLbJAQjEtskBCKmdygEIqKPKAQi5pcoBCLGnygEI4qjKAQjxqcoBCJetygEIza3KAQ==");
    ...
    public String construct_url(String tkk, String q, String from, String to) {
        try {
            String base = url + "?client=webapp&dt=at&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm" +
                "&dt=ss&dt=t&otf=2&ssel=0&tsel=0&kc=1&tk=" + tkk + "&q=" + q + "&sl=" + from +
                "&tl=" + to;
            Log.e("TAG", "=====base======" + base);
            return base;
        } catch (Exception e) {
